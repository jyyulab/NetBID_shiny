...
server <- function(input, output,session) {
  ...
  shiny::observeEvent(input$initButton0, { 
    #js$refresh();
    control_para$doloadData <- FALSE; control_para$doplot <- FALSE;})
  ...
  loadDemoData <- shiny::reactive({
    ...
    analysis.par$out.dir.DATA <- 'data/project_RData/Demo.analysis.par.Step.ms-tab.RData'
  ...
  output$ms_table <- DT::renderDataTable({
    ...
    if(control_para$doloadData=='doinitialload' | control_para$doloadData=='doinitialdemoload'){
      ...
      if(is.null(use_data$all_gs2gene)==TRUE | input$use_spe != use_spe){
        gs.preload(use_spe=input$use_spe,db.dir='db/')
      ...
      if(is.null(use_data$transfer_tab)==TRUE | input$use_level != use_level | from_type!=ori_from_type){
        ...
        db.preload(use_spe=input$use_spe,use_level=input$use_level,db.dir='db/')
